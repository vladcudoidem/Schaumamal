...
class DisplayDataResolver(
    ...
    fun resolve(dumpsDirectoryName: String, selectedDump: Dump): List<DisplayData> {
        val currentDumpDirectoryPath =
            appDirectoryPath.resolve(dumpsDirectoryName).resolve(selectedDump.directoryName)

        val dumpFile = currentDumpDirectoryPath.resolve(selectedDump.xmlTreeFileName).toFile()
        val displayNodes = xmlParser.parseSystem(dumpFile)

        return selectedDump.displays.map { display ->
            DisplayData(
                screenshotFile =
                    currentDumpDirectoryPath.resolve(display.screenshotFileName).toFile(),
